    ...
    def get_option_chain(self, symbol: str, expiry: Optional[str] = None) -> Dict:
        ...
        def fetch_with_fallback():
            try:
                ...
                nse = get_nse_live()
                return nse.equities_option_chain(symbol) if is_index else nse.index_option_chain(symbol)

        try:
            raw = self._cached(f'chain_{symbol}', lambda: _call_with_retry(fetch_with_fallback))
        except Exception as e:
        ...
        if not rows:
            return {'error': f'No option chain data available for {symbol}'}
...
class IndiaRiskFreeRateProvider:
    ...
    the same way ^TNX is used as a blunt US-10Y proxy elsewhere in this app."""
    ...
    def get_rates(self) -> Dict:
        try:
            ...
            return {'error': f"Failed to fetch RBI rates: {str(e)}"}

        if not raw:
            return {'error': 'RBI rate data unavailable'}
